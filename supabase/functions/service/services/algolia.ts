...
const log = logger.child({ namespace: "services.algolia" });
...
export const searchAlgolia = async (query: string): Promise<PromptSearch[]> => {
  try {
    ...
    log.debug("Search Results:", searchResults.hits);

    return searchResults.hits.map((hit: unknown) =>
      PromptSearch.fromJSON(PromptSearch, hit as Record<string, unknown>)
    );
  } catch (error) {
